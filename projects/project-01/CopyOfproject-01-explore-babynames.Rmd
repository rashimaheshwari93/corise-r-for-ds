...
```{r bonus-question-3-transform-2}
tbl_names_by_letter = tbl_names |> 
  # Group by year, sex and first_letter
...
```{r bonus-question-3-visualize-1}
tbl_names_by_letter |> 
  # Filter for the year 2020
   filter( year == 2020) |>
  # Initialize a ggplot of pct_births vs. first_letter
  ...
  facet_wrap(~ sex) +
  # Add labels (title, subtitle, x, y)
  labs(title = "Distribution of births for the year 2020", subtitle = "Based on last letter", x = "Last Letter", y = "Percentage of Births") +
  # Fix scales of y axis
  scale_y_continuous(
    expand = c(0, 0),
    labels = scales::percent_format(accuracy = 1L)
...
```{r bonus-question-3-visualize-2}
plot_trends_in_letter <- function(my_letter) {
  tbl_names_by_letter |> 
    # Filter for first_letter = my_letter
...
tbl_names_by_first_and_last_letter |> 
  ...
  labs(
      title = glue::glue("Distribution of percentage of births by First and Last Letter"),
...
tbl_names_vowel_consonant <- tbl_names |> 
 ...
 mutate(first_last = first_letter_type, last_letter_type, sep = " ")
...
tbl_names_vowel_consonant |> 
  ...
  labs(
      title = glue::glue("Trends of usage of vowels and consonants in names over time"),
      subtitle = "For First and Last letters of the name",
      x = "Year",
      y = "Percentage of births",
      fill = "First and Last letters of the name" 
    ) +
