...
tbl_names <- readr::read_csv(file = here::here("data/names.csv.gz"), show_col_types = FALSE)
...
tbl_lifetables <- readr::read_csv(
  file = here::here ("data/lifetables.csv.gz"), show_col_types = FALSE)
...
tbl_lifetables_extended_2022 |> 
 ...
 geom_line() +
  ...
  labs(
    title = "Probability that a person has survived till 2022",
    subtitle = "Based on the year they were born",
    x = "Year",
    y = "Probability of being alive",
    caption = "Source: SSA"
...
tbl_names_extended <- tbl_names |> 
  ...
  mutate(nb_alive = nb_births * p_alive)
...
ggplot(aes(x = year)) +
    ...
    geom_col(aes(y = nb_alive, fill = "darkgreen", color = "white")) +
    ...
    labs(
      ...
      subtitle = "Based on their name and sex",
      x = "Year",
      y = "Number of people alive"
    ) +
...
tbl_names_extended_age |> 
  ...
  # Add labels (title, subtitle, x, y)
  labs(
    title = "Median Age based on their Name",
    subtitle = "Representing the 25th and the 75th Percentile",
    x = "Median Age",
    y = "Name"
  ) +
